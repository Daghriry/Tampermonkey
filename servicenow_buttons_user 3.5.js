...
// @version      3.5
// @description  Add "Requests", "Assets", "Accessories", "Incidents" buttons, Lenovo warranty via daghriry.info API, accessory name dropdown, and @ mention button in comment box
// @author       You
...
// @match        https://support.qiddiya.com/incident.do*
// @match        https://support.qiddiya.com/now/nav/ui/classic/params/target/incident.do*
// @match        https://support.qiddiya.com/now/nav/ui/classic/params/target/incident_list.do*
...
(function () {
    ...
    function findIncidentCallerInput(doc) {
        if (!doc) return null;
        const selectors = [
            'input#sys_display\\.incident\\.caller_id',
            'input[name="sys_display.incident.caller_id"]',
            '#element\\.incident\\.caller_id input'
        ];
        for (const sel of selectors) {
            const el = doc.querySelector(sel);
            if (el) return el;
    ...
    function openIncidentsForUser(name) {
        const n = (name || '').trim();
        if (!n) { alert('User name is empty.'); return; }
        window.open(
            'https://support.qiddiya.com/now/nav/ui/classic/params/target/' +
            encodeURIComponent(
                'incident_list.do?sysparm_query=' +
                encodeURIComponent('caller_id.nameSTARTSWITH' + n) +
                '&sysparm_first_row=1&sysparm_view=&sysparm_choice_query_raw=&sysparm_list_header_search=true'
            ),
            '_blank'
        );
    ...
    function tryAddButtons() {
        ...
        if (targetInput) {
            ...
            if (!doc.getElementById('qiddiya-user-requests-btn')) {
                ...
                const btnInc = doc.createElement('button');
                btnInc.id = 'qiddiya-user-incidents-btn';
                btnInc.type = 'button';
                btnInc.textContent = 'View user incidents';
                btnInc.title = 'Open all INC incidents raised by this user';
                styleButton(btnInc, '#E11D48', '#BE123C');
                ...
                btnInc.addEventListener('click', () => openIncidentsForUser(getName()));
                ...
                if (targetInput.parentElement) {
                    ...
                    targetInput.parentElement.appendChild(btnInc);
                } else {
                    ...
                    btnAcc.insertAdjacentElement('afterend', btnInc);
                }
            }
        }
